-- DROP TABLE IF EXiSTS tutorial.mz_portfolio_lcs_by_cnline_and_novelty;
-- CREATE TABLE tutorial.mz_portfolio_lcs_by_cnline_and_novelty AS

delete from tutorial.mz_portfolio_lcs_by_cnline_and_novelty;  -- for the subsequent update
insert into tutorial.mz_portfolio_lcs_by_cnline_and_novelty
...
sales_plan_table AS (
SELECT cn_line,
       CASE WHEN extract('year' FROM cn_lcs_launch_date) = extract('year' from current_date) THEN 'novelty' ELSE 'existing' END AS if_novelty,
...
SELECT sales.cn_line,
        CASE WHEN cn_lcs_launch_date >= '2024-12-25' THEN 'novelty' ELSE 'existing' END AS if_novelty,
...
ttl_sales_LY AS (
SELECT sales.cn_line,
       CASE WHEN bu_cn_launch_date >= '2023-12-25' THEN 'novelty' ELSE 'existing' END AS if_novelty,
       COUNT(DISTINCT parent_order_id)                                                                                               AS transactions, 
       sum(case when sales_qty > 0 then order_rrp_amt else 0 end) - sum(case when sales_qty < 0 then abs(order_rrp_amt) else 0 end)  AS sales_rrp
FROM dm_view.offline_lcs_cs__by_sku_fnl sales
WHERE 1 = 1 
...
member_KPI_LY AS (
  SELECT  CASE WHEN bu_cn_launch_date >= '2023-12-25' THEN 'novelty' ELSE 'existing' END                                          AS if_novelty,
          trans.cn_line,
          NULLIF((count(distinct case when is_member_order = TRUE AND if_eff_order_tag = true then trans.member_detail_id else null end)),0)                                                                                   AS member_shopper,
          CAST((sum(case when is_member_order IS TRUE AND sales_qty > 0 then order_rrp_amt else 0 end) - sum(case when is_member_order IS TRUE AND sales_qty < 0 then abs(order_rrp_amt) else 0 end)) AS FLOAT)                AS member_sales,
          CAST((sum(case when is_member_order IS TRUE AND sales_qty > 0 then order_rrp_amt else 0 end) - sum(case when is_member_order IS TRUE AND sales_qty < 0 then abs(order_rrp_amt) else 0 end)) AS FLOAT)/NULLIF(count(distinct case when is_member_order IS TRUE AND if_eff_order_tag = true then trans.parent_order_id else null end),0)             AS member_atv,
...
member_KPI_TY.member_shopper,
      ...
      member_KPI_TY.lifetime_repurchase_member_shopper_share - member_KPI_LY.lifetime_repurchase_member_shopper_share  
FROM sales
...
grant select on tutorial.mz_portfolio_lcs_by_cnline_and_novelty to lego_bi_group;
