-- DROP TABLE IF EXiSTS tutorial.mz_portfolio_lcs_by_cnline_and_sku;
-- CREATE TABLE tutorial.mz_portfolio_lcs_by_cnline_and_sku AS

delete from tutorial.mz_portfolio_lcs_by_cnline_and_sku;  -- for the subsequent update
insert into tutorial.mz_portfolio_lcs_by_cnline_and_sku
...
SELECT sales.cn_line,
       ...
       CASE WHEN min_age_mark <=5 THEN '0-5'
        ...
        END                                                                                                                          AS age_mark_min_group,
       product_cte.product_rrp_price_range                                                                                           AS product_rrp_price_range,
...
WHERE 1 = 1 
 ...
 SELECT
       'TTL' AS cn_line,
       'TTL' AS lego_sku_id,
       'TTL' AS lego_sku_name_cn,
       'TTL' AS if_novelty,
       'TTL' AS age_mark_min_group,
...
SELECT ttl_sales_TY.cn_line,
       ttl_sales_TY.lego_sku_id,
       ttl_sales_TY.lego_sku_name_cn,
       ttl_sales_TY.if_novelty,
       ttl_sales_TY.age_mark_min_group,
       ...
       CAST(ttl_sales_TY.sales_rrp AS FLOAT)/(SELECT SUM(sales_rrp) FROM ttl_sales_TY  WHERE lego_sku_id <> 'TTL')                                      AS sales_rrp_share,
...
LEFT JOIN ttl_sales_LY
       ON ttl_sales_TY.lego_sku_id = ttl_sales_LY.lego_sku_id
      AND ttl_sales_TY.cn_line = ttl_sales_LY.cn_line
LEFT JOIN sales_plan_table
       ON ttl_sales_TY.lego_sku_id = sales_plan_table.lego_sku_id
...
LEFT JOIN member_KPI_TY
      ON sales.lego_sku_id = member_KPI_TY.lego_sku_id
     AND sales.cn_line = member_KPI_TY.cn_line
LEFT JOIN member_KPI_LY
       ON sales.lego_sku_id = member_KPI_LY.lego_sku_id
     AND sales.cn_line = member_KPI_LY.cn_line;

       
grant select on tutorial.mz_portfolio_lcs_by_cnline_and_sku to lego_bi_group;
