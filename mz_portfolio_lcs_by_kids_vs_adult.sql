...
sales_plan_table AS (
SELECT CASE WHEN adult_13_to_17.lego_sku_id IS NOT NULL THEN 'adult_13_to_17' 
            WHEN adult_18_plus.lego_sku_id IS NOT NULL THEN 'adult_18_plus' 
            ELSE 'kids' END                                                          AS adult_vs_kids,
       SUM(rsp*plan_qty)                                                             AS sales_plan
 FROM sales_plan_cte
 LEFT JOIN (SELECT lego_sku_id FROM product_cte 
        WHERE CASE 
        WHEN min_age_mark ~ '^[0-9]+$' THEN min_age_mark::INTEGER >= 13 AND min_age_mark::INTEGER <= 17
        ELSE FALSE
      END) adult_13_to_17
      ON sales_plan_cte.lego_sku_id = adult_13_to_17.lego_sku_id
LEFT JOIN (SELECT lego_sku_id FROM product_cte 
        WHERE CASE 
        WHEN min_age_mark ~ '^[0-9]+$' THEN min_age_mark::INTEGER >= 18
        ELSE FALSE
      END) adult_18_plus
      ON sales_plan_cte.lego_sku_id = adult_18_plus.lego_sku_id
 GROUP BY 1
 UNION ALL
...
LEFT JOIN (SELECT lego_sku_id FROM product_cte 
        ...
        ELSE FALSE
      END) adult_13_to_17
      ON sales.lego_sku_id = adult_13_to_17.lego_sku_id
LEFT JOIN (SELECT lego_sku_id FROM product_cte 
        WHERE CASE 
        WHEN min_age_mark ~ '^[0-9]+$' THEN min_age_mark::INTEGER >= 18
        ELSE FALSE
...
sales AS (
SELECT ttl_sales_TY.adult_vs_kids,
       ...
       CAST(ttl_sales_TY.sales_rrp AS FLOAT)/(SELECT SUM(sales_rrp) FROM ttl_sales_TY WHERE adult_vs_kids <> 'TTL')   AS sales_rrp_share,
...
LEFT JOIN ttl_sales_LY
       ON ttl_sales_TY.adult_vs_kids = ttl_sales_LY.adult_vs_kids
LEFT JOIN sales_plan_table
       ON ttl_sales_TY.adult_vs_kids = sales_plan_table.adult_vs_kids
),
...
LEFT JOIN (SELECT lego_sku_id FROM product_cte 
        ...
        ELSE FALSE
      END) adult_13_to_17
      ON tr.lego_sku_id = adult_13_to_17.lego_sku_id
LEFT JOIN (SELECT lego_sku_id FROM product_cte 
        WHERE CASE 
        WHEN min_age_mark ~ '^[0-9]+$' THEN min_age_mark::INTEGER >= 18
        ELSE FALSE
      END) adult_18_plus
      ON tr.lego_sku_id = adult_18_plus.lego_sku_id
LEFT JOIN (
