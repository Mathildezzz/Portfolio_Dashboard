...
DELETE FROM tutorial.mz_member_sku_count_distribution;
INSERT INTO tutorial.mz_member_sku_count_distribution
...
LEFT JOIN edw.d_dl_product_info_latest product
  ...
  cte AS (
    SELECT omni_channel_member_id,
          ...
          CAST((count(distinct case when is_member_order IS TRUE AND if_eff_order_tag = true then trans.lego_sku_id else null end)) AS FLOAT)   AS sku_count
...
SELECT CASE wHen sku_count >= 10 THEN '10+' ELSE CAST(sku_count AS text) end as sku_count,
